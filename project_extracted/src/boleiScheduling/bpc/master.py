...
class RestrictedMasterProblem:
    """
    Vehicle-slot Dantzig-Wolfe restricted master with optional BSS cuts and
    slot-specific arc-flow branch restrictions.
    ...
    This is equivalent to f^k_ij=0/1 branching because each slot satisfies
        sum_r x[k,r] = 1.
    Hence pricing keeps the same reduced-cost formula, but its route universe
    is restricted in exactly the same way.
    """

    def __init__(self, modelData, columns, bssCuts=None, branchingState=None):
        self.data = modelData
    ...
    def getDuals(self):
        ...
        return MasterDuals(coverage=coverage, slot=slot, makespan=makespan)
